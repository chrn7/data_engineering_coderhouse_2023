...
#Comento los pasos anteriores de "try" y "except" para hacer más productivo el script en el archivo .py, y hago esos pasos de la siguiente forma (mismo código que antes pero sin el try y except, y sin los prints) para hacer más productivo el script:
conn = psycopg2.connect(
        host='data-engineer-cluster.cyhh5bfevlmn.us-east-1.redshift.amazonaws.com',
        dbname=data_base,
        user=user,
...
#consultando la tabla canciones (comento este paso para hacer más productivo el script en el archivo .py):
#cur = conn.cursor()
...
# Veo cómo quedó la tabla en Redshift luego de hacer los Insert, y guardo a los resultados de la tabla en la variable "results":
#consultando la tabla y guardando los resultados en la variable "result":
cur = conn.cursor()
cur.execute("SELECT * FROM videos")
results = cur.fetchall()
